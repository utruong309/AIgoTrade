...
import logging
from trading.models import NewsArticle
from .cache_service import news_cache # type: ignore
...
class NewsService:
    ...
    def fetch_news_for_symbol(self, symbol: str, force_refresh: bool = False) -> List[Dict]:
        ...
        if not force_refresh:
            cached_news = news_cache.get_news(symbol)
            if cached_news:
                logger.info(f"Using Redis cached news for {symbol}")
                return cached_news
            else:
                logger.info(f"No Redis cache found for {symbol}")
        
        if not force_refresh and NewsArticle.is_cache_valid(symbol):
            logger.info(f"Using database cached news for {symbol}")
            db_news = self._get_cached_news(symbol)

            if db_news:
                logger.info(f"Caching database news in Redis for {symbol}")
                news_cache.set_news(symbol, db_news)
            return db_news
        ...
        try:
            ...
            if articles:
                ...
                db_news = self._get_cached_news(symbol)
                if db_news:
                    logger.info(f"💾 Caching fresh news in Redis for {symbol}")
                    news_cache.set_news(symbol, db_news)
                return db_news
            else:
    ...
    def cleanup_old_cache(self, days: int = 7):
        try:
            ...
            try:
                news_cache.cache_service.invalidate_pattern("news_*")
                logger.info("Cleared Redis news cache")
            except Exception as e:
    ...
    def invalidate_cache(self, symbol: str = None):
        try:
            if symbol:
                news_cache.invalidate_news(symbol)
                logger.info(f"Invalidated Redis cache for {symbol}")
            else:
                news_cache.cache_service.invalidate_pattern("news_*")
                logger.info("Invalidated all Redis news cache")
        except Exception as e:
